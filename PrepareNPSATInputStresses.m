...
[sumSTM, sumWLL, totDays, opt] = CVHMInputRCH(startTime, endTime, opt);

CVHMInputHEAD(startTime, endTime, opt);

avSTRM = sumSTM/totDays;
CVHMInputStreamA(avSTRM, startTime, endTime, opt);
% Run the CVHM_HOU.hipnc animation to generate the streams
% Then run the next line to write the input file
CVHMInputStreamB(startTime, endTime, opt);

avWells = sumWLL/totDays;
CVHMInputWellsInit(avWells, startTime, endTime, opt);